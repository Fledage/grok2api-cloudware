...
export interface GrokSettings {
  ...
  imagine_public_image_proxy?: boolean;
}
...
export function buildSsoCookie(token: string, settings: Pick<GrokSettings, "cf_clearance" | "cf_cookies">): string {
  const tok = token.trim().startsWith("sso=") ? token.trim().slice("sso=".length) : token.trim();
  let cookie = `sso=${tok}; sso-rw=${tok}`;
  const fullCookies = sanitizeCookieString(settings.cf_cookies ?? "");
  const clearance = stripCfPrefix(settings.cf_clearance ?? "");
  if (fullCookies) {
    const withoutSso = fullCookies
      .split(";")
      .map((part) => part.trim())
      .filter((part) => part && !/^sso(?:-rw)?=/i.test(part))
      .filter((part) => (clearance ? !/^cf_clearance=/i.test(part) : true))
      .join("; ");
    if (withoutSso) cookie += `; ${withoutSso}`;
  }
  if (clearance) cookie += `; cf_clearance=${clearance}`;
  return cookie;
}
...
export async function getSettings(env: Env): Promise<SettingsBundle> {
  ...
  const mergedGrok = {
    ...
    cf_cookies: sanitizeCookieString(grokCfg.cf_cookies ?? ""),
    user_agent: String(grokCfg.user_agent ?? DEFAULTS.grok.user_agent).trim() || DEFAULTS.grok.user_agent,
  };
...
export async function saveSettings(
  ...
  const nextGrok: GrokSettings = {
    ...
    cf_cookies: sanitizeCookieString(updates.grok_config?.cf_cookies ?? current.grok.cf_cookies ?? ""),
    user_agent: String(updates.grok_config?.user_agent ?? current.grok.user_agent ?? DEFAULTS.grok.user_agent).trim() || DEFAULTS.grok.user_agent,
  };
