...
- **模型集合与主 README 对齐**：同步 `chenyme/grok2api` 最新 `grok-4.20-*`、`grok-4.3-beta` 模型目录，并保留旧 Workers 模型名作为兼容别名
...
- Node.js 20+（推荐 Node.js 22+；本仓库锁定的 Wrangler 需要新版 Node）
...
- Admin APIs: /api/*
- Upstream WebUI pages:
  - GET /webui/login
  - GET /webui/chat
  - GET /webui/masonry
  - GET /webui/chatkit
- Upstream WebUI APIs:
  - GET /webui/api/models
  - POST /webui/api/chat/completions
  - GET /webui/api/imagine/ws
  - POST /webui/api/voice/token
- Upstream admin compatibility APIs:
  - /admin/api/tokens*
  - /admin/api/assets*
  - /admin/api/cache*
  - /admin/api/batch/*
